    ...
    for name in sorted(servers):
        ...
        if kind == "stdio":
            if "command" not in spec:
                raise SyncError(f"{source_rel}: server {name!r}: a local server needs a command")
            if not keys <= {"command", "args", "env"}:
                extra = ", ".join(sorted(keys - {"command", "args", "env"}))
                raise SyncError(f"{source_rel}: server {name!r}: key(s) not rendered: {extra}")
            _no_expansion(source_rel, name, spec)
            out.append(f"command = {toml_basic(spec['command'])}")
            args = spec.get("args", [])
            if not isinstance(args, list) or not all(isinstance(a, str) for a in args):
                raise SyncError(f"{source_rel}: server {name!r}: args must be a list of strings")
            out.append("args = [" + ", ".join(toml_basic(a) for a in args) + "]")
            env = spec.get("env", {})
            if env:
                if not isinstance(env, dict) or not all(isinstance(v, str) for v in env.values()):
                    raise SyncError(f"{source_rel}: server {name!r}: env must map names to strings")
                out.append("")
                out.append(f"[mcp_servers.{name}.env]")
                for k in sorted(env):
                    out.append(f"{k} = {toml_basic(env[k])}")
        elif kind in ("http", "sse"):
            if "url" not in spec:
                raise SyncError(f"{source_rel}: server {name!r}: a remote server needs a url")
...
def role_sources(root):
    ...
    for path in sorted(agents.glob("*.md")):
        ...
        if not name:
            raise SyncError(f"{rel}: no name in the frontmatter")
...
def _role_outputs(root, adapter, block, outputs):
    ...
    for name, (rel, fields, body, skills) in roles.items():
        ...
        out_rel = f"{block['dir'].rstrip('/')}/{name}.toml"
...
def _mcp_outputs(root, adapter, block, outputs):
    ...
    try:
        head = (root / head_rel).read_text(encoding="utf-8")
    except OSError as exc:
        raise SyncError(f"{head_rel}: cannot be read, {type(exc).__name__}")
    if head and not head.endswith("\n"):
        head += "\n"
    outputs[block["path"]] = head + "\n" + render(servers, source_rel)
...
def _orphans(root, outputs):
    ...
    for adapter, manifest in load_manifests(root):
        ...
        if not block or "dir" not in block:
            continue
        directory = root / block["dir"]
